...
class main_window(QMainWindow, MainWindow_poverki.Ui_MainWindow):
    ...
    def __init__(self, parent = None):
        ...
        self.pushButton_5.clicked.connect(self.readFile)
        self.pushButton_4.clicked.connect(self.validate)

    def validate(self):
        filepath = 'D:/load_testing/log/import.2020-04-14.xsd'
        file_name = os.path.basename(filepath)

        self.lineEdit_21.setText(file_name)

        schema_xsd = open(filepath, encoding='utf-8').read()
        schema = xmlschema.XMLSchema(schema_xsd)

        try:
            schema.validate('D:/load_testing/test_500/5000/08-07_load_500_5000_1_signCipher_М.xml')
            print('Ошибок в файле нет')
            self.listWidget.addItem('Ошибок в файле нет')
        except XMLSchemaChildrenValidationError as e:
            print('Ошибка:', e)
            self.listWidget.addItem(e.message)
            self.listWidget.addItem(e.reason)
        except ParseError as parse:
            print(parse)
            self.listWidget.addItem(parse.msg)

    def readFile(self):
        file = QFileDialog.getOpenFileName(self, "Выбрать xml-заявку")
        return file
